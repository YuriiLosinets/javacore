...
public class Solution {
    ...
    public static boolean isPointInPolygon(Point point, List<Point> polygon)
    {
        ...
        {
            if ((polygon.get(i).y > point.y) != (polygon.get(j).y > point.y) && (point.x < (polygon.get(j).x - polygon.get(i).x) * (point.y - polygon.get(i).y)/ (polygon.get(j).y - polygon.get(i).y) + polygon.get(i).x))
            {
                result = !result;
            }
        }
